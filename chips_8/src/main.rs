...
fn main() {
    ...
    loop {
        ...
        if chips_8_state.draw {
            ...
            (0..32).for_each(|y| {
                (0..64).for_each(|x| {
                    print!("{}", chips_8_state.gfx[y * 64 + x]);
                });
                print!("\n");
            })
        }
    ...
    //                     println!("Total Flipped: {}", texture.pixels.iter().step_by(4).enumerate().fold(0, |acc, (idx, _)| {
    //                         if texture.pixels[idx] == 255 && texture.pixels[idx + (1 as usize)] == 255 && texture.pixels[idx + (2 as usize)] == 255 && texture.pixels[idx + (3 as usize)] == 255 {
    //                             acc + 1
    //                         } else {
    //                             acc
    //                         }
