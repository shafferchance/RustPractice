use std::{vec::Vec, fmt::{Display, format}};

use super::utils::{get_nnn, get_vx, get_kk, get_vy, get_nibble};
...
impl DisassembledChip8 {
    pub fn new(bytes: &[u8]) -> DisassembledChip8 {
        let op_tuple_vec = bytes.iter().step_by(2).enumerate().map(|(idx, b_val)| {
...
fn get_disassembled_op_tuple(idx: &usize, op: &u16) -> DisassembledOpTuple {
    match op & 0xF000 {
        ...
        0x3000 => {
            return create_disassembled_op_tuple_desc_string(idx, op, "SE", format!("V{:X}, {:X}", get_vx(op), get_kk(op)), Some("Vx == kk"));
        }

        // 0x4xkk - SNE Vx, byte - Skip next if Vx != kk
        0x4000 => {
            return create_disassembled_op_tuple_desc_string(idx, op, "SNE", format!("V{:X}, {:X}", get_vx(op), get_kk(op)), Some("Vx != kk"));
        }

        // 0x5xy0 - SE Vx, Vy - Skip next if Vx == Vy
        0x5000 => {
            return create_disassembled_op_tuple_desc_string(idx, op, "SE", format!("V{:X}, {:X}", get_vx(op), get_vy(op)), Some("Vx == Vy"));
        }

        // 0x6xkk - LD Vx, byte - Load kk into Vx
        0x6000 => {
            return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("V{:X}, {:X}", get_vx(op), get_kk(op)), None)
        }

        // 0x7xkk - ADD Vx, byte - Add value of byte
        0x7000 => {
            return create_disassembled_op_tuple_desc_string(idx, op, "ADD", format!("V{:X}, {:X}", get_vx(op), get_kk(op)), None)
        }

        0x8000 => match op & 0x000F {
            // LD Vx, Vy
            0x0000 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("V{:X}, V{:X}", get_vx(op), get_vy(op)), None)
            }
            // OR Vx, Vy
            0x0001 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "OR", format!("V{:X}, V{:X}", get_vx(op), get_vy(op)), None)
            }
            // AND Vx, Vy
            0x0002 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "AND", format!("V{:X}, V{:X}", get_vx(op), get_vy(op)), None)
            }
            // XOR Vx, Vy
            0x0003 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "XOR", format!("V{:X}, V{:X}", get_vx(op), get_vy(op)), None)
            }
            // ADD Vx, Vy - Add Vx to Vy
            0x0004 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "ADD", format!("V{:X}, V{:X}", get_vx(op), get_vy(op)), None)
            }
            // SUB Vx, Vy - Subtract Vx from Vy
            0x0005 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "SUB", format!("V{:X}, V{:X}", get_vx(op), get_vy(op)), None)
            }
            // SHR Vx - RHS 1
            0x0006 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "SHR", format!("V{:X}", get_vx(op)), None)
            }
            // SUBN Vx, Vy - Subtract Vy from Vx
            0x0007 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "SUBN", format!("V{:X}, V{:X}", get_vx(op), get_vy(op)), None)
            }
            // SHL Vx - LHS 1
            0x000E => {
                return create_disassembled_op_tuple_desc_string(idx, op, "SHL", format!("V{:X}", get_vx(op)), None)
            }
        ...
        0x9000 => {
            return create_disassembled_op_tuple_desc_string(idx, op, "SNE", format!("V{:X}, V{:X}", get_vx(op), get_vy(op)), None)
        }
        ...
        0xC000 => {
            return create_disassembled_op_tuple_desc_string(idx, op, "RND", format!("V{:X}, {:X}", get_vx(op), get_nnn(op)), None)
        }

        // 0xDxyn - DRW Vx, Vy, nibble - Draw n-byte sprite starting at mem loc I @ (vx, Vy), set VF = collision
        0xD000 => {
            return create_disassembled_op_tuple_desc_string(idx, op, "DRW", format!("V{:X}, V{:X}, {:X}", get_vx(op), get_vy(op), get_nibble(op)), None)
        }

        0xE000 => match op & 0x00FF {
            // 0xEx9E - SKP Vx - Skip next instruction if key pressed
            0x009E => {
                return create_disassembled_op_tuple_desc_string(idx, op, "SKP", format!("V{:X}", get_vx(op)), None)
            }

            // 0xExA1 - SKNP Vx - Skip next instruction if key not pressed
            0x00A1 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "SKNP", format!("V{:X}", get_vx(op)), None)
            }
            _ => create_disassembled_op_tuple(idx, op, "X", "Unsupported op found"),
        },
        ...
        0xF000 => match op & 0x00FF {
            ...
            0x0007 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("V{:X}, DT", get_vx(op)), Some("DT - Delay Timer"))
            }
            // 0xFx0A - LD Vx, K - Stop execution till key press
            0x000A => {
                return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("V{:X}, K", get_vx(op)), Some("Stop execution till keypress"))
            }
            // 0xFx15 - LD DT, Vx - Load Vx into DT
            0x0015 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("DT, V{:X}", get_vx(op)), None)
            }
            // 0xFx18 - LD ST, Vx - Set sound timer to Vx
            0x0018 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("ST, V{:X}", get_vx(op)), Some("ST - Sound Timer"))
            }
            // 0xFx1E - ADD I, Vx - Add I and Vx then store in I
            0x001E => {
                return create_disassembled_op_tuple_desc_string(idx, op, "ADD", format!("I, V{:X}", get_vx(op)), None)
            }
            // 0xFx29 - LD F, Vx - Set I = location of sprite for digit Vx
            0x0029 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("F, V{:X}", get_vx(op)), Some("Load font"))
            }
            // 0xFx33 - LD F, Vx - set_BCD
            0x0033 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("F, V{:X}", get_vx(op)), Some("Set BCD"))
            }
            // 0xFx55 - LD [I], Vx -reg_dump
            0x0055 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("[I], V{:X}", get_vx(op)), Some("Reg Dump"))
            }
            // 0xFx65 - LD Vx, [I] - reg_load
            0x0065 => {
                return create_disassembled_op_tuple_desc_string(idx, op, "LD", format!("V{:X}, I", get_vx(op)), None)
            }
            _ => create_disassembled_op_tuple(idx, op, "X", "Unsupported op found"),
        },
